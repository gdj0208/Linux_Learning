...
**[ 설명 ]**
- 부모-자식 프로세스 간에 통신을 할 수 있게 함
- `부모 프로세스 -> 자식 프로세스` 또는 `자식 프로세스 -> 부모 프로세스` 중 한 방향을 선택
- 파이프를 이용해 양방향 통신을 원할 경우 파이프 2개를 생성해야 함
...
- 자식 프로세스와 파이프를 만들고 `mode`에 따라 표준 입출력을 연결
...
```c
void FD_ZERO(fd_set *fdset);            // fd_set이 가리키는 마스크 초기화
void FD_SET(int fd, fd_set *fdset);     // fd_set이 가리키는 마스크 내 비트를 1로 설정
int FD_ISSET(int fd, fd_set *fdset);    // fd_set이 가리키는 마스크 내 비트가 설정되어있는가?
void FD_CLR(int fd, fd_Set *fdset);     // fd_Set이 가리키는 마스크 내 비트를 0으로 설정
```
...
## 4️⃣ 레코드 록킹
### 🎯 레코드 록킹이란?
- 복수의 프로그램이 동시에 같은 파일의 특정 부분에 접근하려 할 때 데이터가 꼬이거나 깨지는 것을 막기 위한 정리
...
```
- `filedes` : 유효한 개방된 파일 기술자
- `cmd` : 
    - `F_GETLK` : ldata를 통해 전달된 lock 정보 획득
    - `F_SETLK` : 파일에 lock 적용
    - `F_SETLKW` : 파일에 lock 적용

**[ ldata ]**
![alt text](image-2.png)
```c
struct flock {
    short l_type;   // lock 유형 : F_RDLCK, F_WRLCK, F_UNLCK
    short l_whence;
    off_t l_start;
    off_t l_len;
    pid_t l_pid;
